"""
今回は未実行。
CB のクラスタ結果を入力に、one-vs-rest で RandomForest を回すための雛形。
"""
...
INPUT_CLUSTERED_CSV = OUTPUT_DIR / "CB_clustered_players.csv"
...
def main() -> None:
    ...
    print(f"[CB][RF] 入力件数: {len(rows)}")
    print(f"[CB][RF] 入力ファイル: {INPUT_CLUSTERED_CSV}")
    print("[CB][RF] 今回は未実行です。TODO を参照してください。")
